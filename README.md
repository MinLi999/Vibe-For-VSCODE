...
> ⚠️ **活跃开发中**：功能仍在快速迭代。已可从 [vibefox.app/download](https://vibefox.app/download) 下载公证过的安装包（内置 Sparkle 自动更新，装一次即可跟上后续版本）。
...
- **语气随目标应用变化** —— 识别当前前台应用，让改写阶段跟着调整（聊天软件保持随意，邮件保持得体）；「风格」页可给任意应用**强制指定语气**，覆盖自动判断。
- **词库越用越准** —— 三条自动生长通路：历史记录里**改错回学**（把听错的改对，误听词自动进词库）、**通讯录导入**（人名/公司名一键入库，仅本机）、**同音字校正**（词库词条自动修正"读音对但字写错"，本地零耗时）。
- **短语模板** —— 说「插入我的邮箱」输出完整地址，说一个口令输出整段模板（本地确定性替换，最长 2000 字符）。
...
1. 下载：[vibefox.app/download](https://vibefox.app/download)（已公证，解压拖进「应用程序」即可）；或从源码构建：`cd macos && ./scripts/make-app.sh`（需要 Xcode）。
2. 打开 VibeFox，跟随新手引导授予**麦克风**和**辅助功能**权限，选好转写引擎，现场练习一次。
