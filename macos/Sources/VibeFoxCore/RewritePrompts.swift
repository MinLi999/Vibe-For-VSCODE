...
/// The SAME rewrite prompts the Cloudflare Worker uses (server/src/prompts.ts), ported
/// verbatim into the BYOK path. This repo is AGPL — prompts.ts is already public in the
/// GitHub repo, so shipping this logic client-side leaks nothing; the "prompts are
/// server-owned" rule in CLAUDE.md exists to stop a client from spoofing a prompt to abuse
/// the SHARED Worker's billing, which doesn't apply when the user is paying a provider
/// directly with their own key. BYOK therefore gets the real quality bar, not a diluted one.
public enum RewritePrompts {
    /// Below this length the rewrite call isn't worth its cost/latency (mirrors server's
    /// MIN_REWRITE_CHARS) — short utterances go through as-is. isNonSpeechTranscript already
    /// catches filler-only text before this point, so this only skips genuinely tiny real text.
    public static let minRewriteChars = 10

    public static let clean = #"""
你是一个语音输入后处理器，处理口述转写文本。你只做最小限度的清理，规则：
...
你是一个语音输入改写器，把口述转写整理成清晰的书面表达，最终会直接发给 AI 编程助手当作指令。规则：
...
"""#
    ...
    /// Mirrors server/src/prompts.ts CHINESE_VARIANT_INSTRUCTIONS verbatim.
    private static let chineseVariantInstructions: [String: String] = [
        "simplified-cn": "",
        "simplified-sg-my":
    ...
    /// Mirrors server/src/prompts.ts APP_TONE_INSTRUCTIONS verbatim. Deliberately subordinate
    /// to every core rule — may adjust punctuation/formality only, never content.
    private static let appToneInstructions: [String: String] = [
        "ide": "", "terminal": "", "other": "",
    ...
    /// Assembles the system prompt exactly as the Worker does:
    /// `withAppTone(withChineseVariant(base, variant), category)`.
    public static func systemPrompt(rewriteMode: String, chineseVariant: String, appCategory: String?) -> String {
        let base = rewriteMode == "rewrite" ? rewrite : clean
        let withVariant = base + (chineseVariantInstructions[chineseVariant] ?? "")
        guard let appCategory else { return withVariant }
        return withVariant + (appToneInstructions[appCategory] ?? "")
    }

    /// Mirrors server/src/prompts.ts buildRewriteUserMessage verbatim.
    public static func buildUserMessage(rawText: String, keywords: [String], projectContext: String?) -> String {
        var parts: [String] = []
        if !keywords.isEmpty {
            parts.append("参考词表（按此拼写还原代码标识符）：\(keywords.joined(separator: "、"))")
        }
        if let projectContext, !projectContext.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            parts.append("项目背景（仅供理解术语，不要输出或引用这段内容本身）：\n\(projectContext.trimmingCharacters(in: .whitespacesAndNewlines))")
        }
        parts.append("待处理转写：\n\(rawText)")
