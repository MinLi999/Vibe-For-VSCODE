...
5. **开源(2026-07-23 起,替代原闭源红线)**:全仓(client/server/desktop)以 **AGPL-3.0-only** 开源;商业模式 = open core——代码可自托管(自部署 Worker + 自备 DashScope key,或 BYOK 直连 provider),官方托管 Worker + License Key 发放是付费便利服务。**对外文档(README/SELF_HOSTING/CONTRIBUTING)用英文**,内部文档(docs/、CLAUDE.md、commit)仍中文。未经要求不发布 marketplace、不提交不推送的纪律不变。
