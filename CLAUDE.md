...
3. **转写语言锁 `zh`**(`vibefox.language` 可覆盖):显式传 `language` 给 Whisper,绕过自动检测延迟。
4. **录音上限 25s**(`vibefox.maxRecordSeconds`):MP3 16kHz 单声道 ~32kbps,payload 远低于 Worker 128MB 内存/30s 执行上限;Worker 端拒收 >8MB base64(413)。
