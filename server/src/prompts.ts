...
7. 如果转写内容明显是说到一半被截断的未完成句子（缺少宾语、动词或结尾，语义不完整），原样保留这个不完整状态，不要猜测、编造或补全说话人没有说出的结尾（例如"给我的文"不要补成"给我的文件"或"给我的文本"，"按住 Mac 的启动的"不要补成"按住 Mac 的 launch 快捷键"——照抄原样即可）。
...
3. 轻度修复语法、断句与标点，可以合并零散语句、微调语序使表达通顺，但绝不改变技术意图，绝不添加说话人没有说出的需求、参数或实现细节；如果转写内容明显是说到一半被截断的未完成句子（缺少宾语、动词或结尾，语义不完整），原样保留这个不完整状态，不要猜测、编造或补全说话人没有说出的结尾（例如"给我的文"不要补成"给我的文件"或"给我的文本"——照抄原样即可）；
