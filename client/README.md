...
2. **服务端**:部署 `server/` 的 Cloudflare Worker,把 URL 填进 `vibefox.endpoint`。
3. **License Key**:命令面板 → `VibeFox: Set License Key`(存 SecretStorage,不进设置文件)。

## 使用

| 操作 | 方式 |
|---|---|
| 开始/停止录音 | `Ctrl+Shift+Space` 或点状态栏 `$(mic) VibeFox` |
| 取消录音 | 录音中按 `Esc` |
| 插入位置 | `vibefox.insertTarget`:`auto`(编辑器光标→活动终端→剪贴板)/`editor`/`terminal`/`clipboard` |

说明:
- **Claude Code CLI**(跑在终端里)可直接注入:焦点不在编辑器时 `auto` 会走 `terminal.sendText`。
- **Copilot Chat / Cline** 的 webview 输入框没有跨扩展写入 API:结果会进剪贴板并提示粘贴。
- 代码词汇准确性:默认开启 `vibefox.contextHint`,自动把当前文件 top-40 标识符 + 最近文件名注入 Whisper 提示词。
