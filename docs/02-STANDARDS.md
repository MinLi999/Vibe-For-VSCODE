...
- **引擎路由**:tier 由 KV metadata `plan:"pro"` 决定;质量档 ASR = `qwen3-asr-flash`(亚太 `dashscope-intl` / 美国 `dashscope-us` + `qwen3-asr-flash-us`,按 `request.cf.continent` 自动路由或 `regionPreference` 手动指定,**6s** 超时,`enable_itn:true`)降级 CF Whisper(**20s** race);改写 = **`qwen-plus`(唯一质量档改写引擎,8s 超时,`temperature:0`,同区域 key)** 降级 `@cf/meta/llama-3.1-8b-instruct` 降级原文;**<10 字符跳过改写**。(Haiku 改写引擎与 compareRewrite 对比功能已于 2026-07-13 移除。)
