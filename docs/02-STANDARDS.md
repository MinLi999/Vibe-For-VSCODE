...
- Whisper 路径 `initial_prompt`:中文引导句 + 顿号拼接词表,UTF-8 预算 **800 字节**(`WHISPER_PROMPT_BUDGET_BYTES`;previousTranscript 已移除)。**Qwen3-ASR context 偏置(2026-07-19 复核官方文档后重新启用)**:官方 Qwen-ASR API 参考明确记载同步接口支持"定制化识别"——messages 首位放 `{role:'system', content:[{text}]}` 携带**背景文本与实体词表**(≤1 万 token),但**不支持指令式/角色式自由文本**。故只发 `keywords.join(', ')` 纯实体列表,**projectContext 绝不进 ASR**(早期塞 free-form 文本曾被模型原样"读出来"当转写结果);服务端 `isContextEcho`(nonspeech.ts)对每个 Qwen 结果做复读检测(归一化后剔除词表,残余 <20% 且长度 ≥12 判定复读),命中按 degenerate 处理降级 Whisper(fallback 原因码 `dashscope_context_echo`)。改写阶段的词表校正照旧(`buildRewriteUserMessage` 携带 keywords + projectContext)。
...
- **引擎路由**:tier 由 KV metadata `plan:"pro"` 决定;质量档 ASR = `qwen3-asr-flash`(亚太 `dashscope-intl` / 美国 `dashscope-us` + `qwen3-asr-flash-us`,按 `request.cf.continent` 自动路由或 `regionPreference` 手动指定,**6s** 超时,`enable_itn:true`,**`language:'auto'` 时不传该参数**——官方对混合语种建议勿指定)降级 CF Whisper(**20s** race);改写 = **`qwen-plus`(唯一质量档改写引擎,8s 超时,`temperature:0`,同区域 key)** 降级 `@cf/meta/llama-3.1-8b-instruct` 降级原文;**<10 字符跳过改写**。(Haiku 改写引擎与 compareRewrite 对比功能已于 2026-07-13 移除。)
...
- Whisper 调用:`@cf/openai/whisper-large-v3-turbo`,`task:"transcribe"`,`language` 显式传(客户端 `auto` 由服务端映射为 `"zh"`,绕过 Whisper 自动检测的延迟与误检),`vad_filter:true`,`temperature:0`。
