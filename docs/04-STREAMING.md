...
- **M3 desktop 接入**:复用同一 service;托盘 title 显示 partial。
  **状态(2026-07-23)**:代码已落地——config.json 新增 `streamingMode`(默认 false);复用 `openTranscriptionStream`(appCategory/vocabulary 随 start 帧);托盘电平旁显示 partial 尾 10 字符;segment → dedupe → 历史 → 粘贴;降级同 M2(整段缓冲 PCM 经 `enqueueSegment` 批量重放,dedupe 裁重)。
  **运行时依赖已解决(同日)**:原 Electron 33 = Node 20 无全局 WebSocket,流式在桌面端无法激活;已升级 **electron 33→43.2.0(Node 24)**、`@types/node` 20→24、`electron-builder` 25→26.15.3。实测:`typeof WebSocket === 'function'` ✅、typecheck+esbuild ✅、独立 userData 启动存活 10s+ 且 config.json 正常生成 ✅。`streamingSupported()` 的一次性降级提示保留为安全网。⚠️ **未验证**:`npm run dist` 打包 + Developer ID 签名(会调用用户钥匙串证书,需用户本人执行);跨大版本升级后建议重新确认托盘图标、热键、辅助功能授权三项(TCC 身份基于证书+bundle id,理论上不受影响)。
